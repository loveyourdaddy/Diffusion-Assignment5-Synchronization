...
class WideImageModel(BaseModel):
    ...
    def forward_mapping(self, z_t, **kwargs):
        ...
        # Projection operation: Cropping windows from canonical space
        for h_start, h_end, w_start, w_end in self.mapper:
            # Crop each window using the mapper coordinates
            x_t = z_t[:, :, h_start:h_end, w_start:w_end]
            xts.append(x_t)
        
        # Stack all cropped windows into a batch
    ...
    def inverse_mapping(self, x_ts, **kwargs):
        ...
        # Reset count and value accumulators
        self.count.zero_()
        self.value.zero_()
        
        # Unprojection operation: Pasting windows back to canonical space
        for i, (h_start, h_end, w_start, w_end) in enumerate(self.mapper):
            # Add each window's values
